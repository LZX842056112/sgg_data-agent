...
在传统企业数据分析场景中，业务人员想要查询数据，必须依赖数据分析师手写 SQL，流程冗长、成本高。**掌柜问数**（Data Agent）解决了这个痛点：

> 用户只需用自然语言提问，系统自动完成对数据仓库的理解、SQL 生成、查询执行，并返回结果。

**核心优势：**

- 不用写 SQL，不需要培训，像聊天一样查数据
- 自动理解表结构，智能生成 JOIN 查询
- 实时流式输出执行进度，用户体验好
- 业务人员自助查数，解放数据团队

![](assets/image-20260121184748743.png)

项目意义：告别手写 SQL，自然语言提问，秒出结果、秒懂经营。让数据会说话，让决策更简单。
...
| 组件           | 技术选型          | 用途                           |
| -------------- | ----------------- | ------------------------------ |
| LLM            | DeepSeek Chat     | 关键词扩展、SQL 生成、SQL 校正 |
| Agent 框架     | LangGraph         | 编排多步骤智能体工作流         |
| 向量数据库     | Qdrant            | 字段与指标的语义检索           |
| 全文搜索       | Elasticsearch     | 字段取值的关键词检索           |
| 关系型数据库   | MySQL             | 元数据存储 + 模拟数据仓库      |
| Embedding 模型 | BGE-large-zh-v1.5 | 文本向量化                     |
| Web 框架       | FastAPI           | 提供 SSE 流式 API              |
| ORM            | SQLAlchemy        | 异步数据库读写                 |
...
![](assets/image-20260208173519231.png) 



数据仓库（DW-Data Warehouse）是**面向分析、非易失性、随时间变化**的**企业级数据存储系统**，核心作用是把企业分散在各个业务系统（如电商的交易系统、物流的仓储系统、CRM 的客户系统、 Elasticsearch/Qdrant 检索库、MySQL 业务库等）的零散数据，经过**抽取、清洗、转换、加载** 后，按**主题域**整合存储，最终支撑企业的**数据分析、报表统计、经营决策、数据挖掘**等场景。
...
简单说：**业务库管 “干活”（支撑日常交易 / 操作），数据仓库管 “复盘”（支撑分析 / 决策）**
...
```
数据仓库 (dw库)  +  meta_config.yaml
        │
        ▼
  MetaKnowledgeService
   ├── 写入 MySQL (meta库)  ← 表信息、字段信息、指标信息
   ├── 写入 Qdrant          ← 字段向量索引、指标向量索引
   └── 写入 Elasticsearch   ← 字段取值全文索引
```

**查询期（在线）：**

```
用户问题 → 智能体 → 读取 Qdrant/ES/MySQL → 生成 SQL → 执行 dw库 → 返回结果
```
...
**事实表（Fact Table）**：存储核心业务的 “度量值”（如订单数量、订单金额），并通过外键关联各个维度表，是分析的 “核心数据”	，在实际使用中事实表主要用于聚合操作。存储核心业务交易数据（“发生了什么”）；

**维度表（Dimension Table）**：用于描述业务的 “维度属性”，比如客户信息、商品信息、时间维度、区域信息，是分析的 “上下文”，在实际使用中维度表主要用于分组和过滤操作。 
...
![](assets/image-20260208173548508.png)
...
具体示例如下图所示：
 ...
 ![](assets/image-20260208173614373.png)
...
![](assets/image-20260208173647157.png)
...
注意:pycharm中使用方便的uv需要在2025版本（创建项目有uv图标）
...
下发虚拟机信息: 192.168.200.10  root/root 
...
在`data-agent/app/clients/mysql_client_manager.py`中编写代码，用来管理MySQL客户端。
...
Hugging Face 文本嵌入推理（Text Embeddings Inference，简称 TEI）是一套用于部署和提供开源文本嵌入与序列分类模型服务的工具包。TEI 可为当下最主流的各类模型实现高性能的特征提取。bedding Inference客户端使用LangChain提供的HuggingFaceEndpointEmbeddings，具体用法参考[官方文档](https://docs.langchain.com/oss/python/integrations/text_embedding/text_embeddings_inference)。向量模型服务创建时使用的是bge-large-zh-v1.5模型。
...
![image-20260207142744497](assets/image-20260208173356129.png)
...
# (2)传参测试
python -m app.scripts.build_meta_knowledge -c 10 
# 输出结果
Namespace(filename='meta.yaml', count='10')
...
~~~shell
python -m app.scripts.build_meta_knowledge meta.yaml -c 10 
结果：
meta.yaml
~~~
...
# 创建客户端对象
embeddings = HuggingFaceEndpointEmbeddings(model="http://192.168.200.10:8081")
...
- TableInfo（data-agent\app\entities\table_info.py）
...
**转换器，负责在两种实体之间互相转换数据，比如：**

> 数据库层（MySQL 模型：`ColumnInfoMySQL`）
>
> 业务逻辑层（实体：`ColumnInfo`）

- ColumnInfoMapper（data-agent\app\mappers\column_info_mapper.py）
...
- ColumnMetricMapper（data-agent\app\mappers\column_metric_mapper.py）
...
- MetricInfoMapper（data-agent\app\mappers\metric_info_mapper.py）
...
- TableInfoMapper（data-agent\app\mappers\table_info_mapper.py）
...
#### 5.3.4.5 column_qdrant_repository
...
#### 5.3.4.6 metric_qdrant_repository
...
```bash
(data-agent) PS D:\workspace\data-agent> python -m app.scripts.build_meta_knowledge -c D:\code\workspace-python\BJ260119\data-agent\conf\meta_config.yaml   
```
...
定义`data-agent/app/agent/node/extract_keywords.py`模块 负责定义关键词抽取的节点
...
![image-20260302160651463](assets/image-20260302160651463-1779027386371.png)
...
阈值 `score_threshold=0.6` 意思是：**只返回相似度 ≥ 0.6 的结果**。为什么是 0.6（而不是 0.5 / 0.7）

经验区间：中文 / 通用场景的舒适区

- **0.5～0.7**：中文业务文档、知识库、RAG 问答最常用区间
- **<0.5**：太松，容易混入大量不相关内容（噪声多、幻觉多）
- **>0.7**：太紧，容易漏掉相关结果（召回低、经常查不到）

以中文常用模型为例 BGE、text-embedding-ada-002、通义千问 Embedding
...
- **强召回场景**（如法律、医疗查全）：降到 **0.45～0.5**
- **高精度场景**（如客服精准问答）：升到 **0.7～0.75**
- **英文 / 小语种、稀疏数据**：通常比中文低，**0.5 左右更常见**

**0.6 不是魔法数字，是中文通用 RAG / 知识库场景下，“精度、召回、防幻觉” 三者平衡的经验最优值**；它挡住低质噪声，又不过度误伤，是工程落地的安全默认线
...
**2）**在data-agent/app/repositories/mysql/meta/meta_mysql_repository.py中添加如下代码：todo
...
![image-20260311204011515](assets/image-20260311204011515-1779027386371.png)
...
![image-20260312162206045](assets/image-20260312162206045-1779027386371.png)
...
![image-20260312164049033](assets/image-20260312164049033-1779027386371.png)
...
![image-20260312185315752](assets/image-20260312185315752-1779027386371.png)
...
![image-20260207161919313](assets/image-20260207161919313-1779027386372.png)
